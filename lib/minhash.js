...
class MinHash {
  ...
  addWord (word) {
    var hashed = mm3.x64.hash128(word, this.seed)
    this.addHash(parseInt(hashed.slice(0, 16), 16))
  }
...
function revcomp (seq) {
  ...
  return Array.prototype.map.call(seq, function (x) {
    return conversion.get(x.toUpperCase())
  }).reverse().join('')
}
...
function getScaledForMaxHash (maxHash) {
  if (maxHash === 0) {
    return 0
  }

  return Math.floor(MINHASH_MAX_HASH / maxHash)
}
...
module.exports = {
  ...
  getMaxHashForScaled: getMaxHashForScaled,
  getScaledForMaxHash: getScaledForMaxHash
}
